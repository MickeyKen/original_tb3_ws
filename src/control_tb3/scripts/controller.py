...
def file_open(file_name):
    if file_name is not None:
        # path = os.path.join('/home', 'turtlebot3-raspi', 'catkin_ws','src','control_tb3','scripts', file_name)
        path = os.path.join('/home', 'turltebot3-raspi', 'catkin_ws','src','motion', file_name)
...
class TextfileController(object):
    def __init__(self):
        ...
        #self.wav_path = rospy.get_param("/textfile_controller/wav_path")
        self.wav_path = "/home/turltebot3-raspi/catkin_ws/src/music/"
        self.result_list = []
        self.repeat = 5
        ...
        for texts in file_content:
            ...
            #forward
                ...
                if exit_flag == 1:
                    ...
                    end_flag = 1
                    return
                volume = int(command_param_set[1])
                set_volume(volume)
                print("volume revel {0}:".format(volume))
                ...
                if exit_flag == 1:
                    ...
                    end_flag = 1
                    return
                wav = command_param_set[1]
                path = self.wav_path + wav
                repeat(path, 1)
                print("play" + wav)
                ...
                wav = command_param_set[1]
                path = self.wav_path + wav
                time = int(command_param_set[2])
                repeat(path, time)
                print("repeat" + wav)

            elif command == "multiplay":
                exit_flag = rospy.get_param("/textfile_controller/exit_flag")
                if exit_flag == 1:
                    rospy.set_param("/textfile_controller/exit_flag", 0)
                    end_flag = 1
                    return
                wav1 = command_param_set[1]
                path1 = self.wav_path + wav1
                wav2 = command_param_set[2]
                path2 = self.wav_path + wav2
                multi_play(path1, path2)
                print("multi  play" + wav1 + "and" + wav2)

            elif command == "led":
                on_off = command_param_set[1]
                if on_off == "on":
                    loop_times = int(command_param_set[2])
                    self.led = [loop_times]
                    i = 3
                    while command_param_set[i] != "":
                        self.led.append(int(command_param_set[i]))
                        i += 1
                    self.led_msg.data = self.led
                    self.led_pub.publish(self.led_msg)
                    print "led on"					
...
def set_volume(revel):
	...
	try:
		if (volume <= 100) and (volume >= 0):
			print(4)
			Popen(["amixer", "-D", "pulse", "sset", "Master",
 				str(volume)+"%"])
	except ValueError:
		pass
